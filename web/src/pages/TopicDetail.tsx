...
export default function TopicDetail() {
  ...
  const getVocabDisplayText = (vocab: Vocabulary, index: number) => {
    ...
    return (
      motherTongueTranslation?.meaning ??
      sourceTranslation?.meaning ??
      vocab.word ??
      `Name of word ${index + 1}`
    );
  };

  const filteredVocabularies = vocabulariesWithTranslations.filter((vocab, index) => {
    const q = searchQuery.trim().toLowerCase();
    if (!q) return true;
    const displayText = getVocabDisplayText(vocab, index).toLowerCase();
    const rawWord = (vocab.word || '').toLowerCase();
    return displayText.includes(q) || rawWord.includes(q);
  });
        ...
        <div className="flex-1 overflow-y-auto p-8 2xl:p-10">
          <div className="grid grid-cols-3 md:grid-cols-5 gap-6">
            {filteredVocabularies.map((vocab, index) => {
                ...
                const displayText = getVocabDisplayText(vocab, actualIndex);
                const motherTongueTranslation = getTranslationForLanguage(vocab.translations, user?.nativeLanguage?.id);
                const sourceTranslation = getTranslationForLanguage(vocab.translations, topic.sourceLanguage?.id);
